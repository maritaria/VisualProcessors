...
namespace VisualProcessors.Forms
{
	...
	public partial class ProcessorForm : Form
	{
		...
		public ProcessorForm(PipelineForm pipelineform, Processor processor, bool useModel)
		{
			...
			if (Processor.Meta.InputTabMode == ProcessorVisibility.Hide)
			{
			...
			if (Processor.Meta.PropertiesTabMode == ProcessorVisibility.Hide)
			{
			...
			if (Processor.Meta.CustomTabMode == ProcessorVisibility.Hide)
			{
			...
			if (Processor.Meta.OutputTabMode == ProcessorVisibility.Hide)
			{
		...
		public void SetLinkMode(LinkMode mode, bool origin)
		{
			...
			switch (mode)
			{
				case LinkMode.InputFirst:
					...
					ConfirmLinkButton.Enabled = (Processor.Meta.OutputTabMode != ProcessorVisibility.Hide) && (Processor.OutputChannelCount > 0);
					break;

				case LinkMode.OutputFirst:
					ConfirmLinkButton.Text = "Link Input";
					ConfirmLinkButton.Enabled = (Processor.Meta.InputTabMode != ProcessorVisibility.Hide) && (Processor.InputChannelCount > 0);
					break;
			}
		}
