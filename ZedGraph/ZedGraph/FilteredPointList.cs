...
namespace ZedGraph
{
	...
	public class FilteredPointList : IPointList
	{
		...
		*/
		...
		/// <summary>
		///  Gets the desired number of filtered points to output. You can set this value by calling
		///  <see cref="SetBounds" />.
		/// </summary>
		public int MaxPts
		{
		...
		///  min=double.MinValue and max=double.MaxValue to get the full range of data. Use
		///  maxPts=-1 to not limit the number of points. Call this method anytime the zoom range is
		///  changed.
		/// </summary>
		...
		public void SetBounds(double min, double max, int maxPts)
		{
			...
			// Make sure the bounded indices are legitimate if BinarySearch() doesn't find the
			// value, it returns the bitwise complement of the index of the 1st element larger than
			// the sought value
		...
		// // assume data points are equally spaced, and calculate the X step size between // each
		// data point double step = (_x[_x.Length - 1] - _x[0]) / (double)_x.Length;

		// if (min < _x[0]) min = _x[0]; if (max > _x[_x.Length - 1]) max = _x[_x.Length - 1];

		// // calculate the index of the start of the bounded range int first = (int)((min - _x[0])
		// / step);

		// // calculate the index of the last point of the bounded range int last = (int)((max -
		// min) / step + first);

		// // Make sure the bounded indices are legitimate first = Math.Max(Math.Min(first,
		// _x.Length), 0); last = Math.Max(Math.Min(last, _x.Length), 0);
