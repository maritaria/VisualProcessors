...
{
	/// <summary>
	///  An enum used to specify the X or Y data type of interest -- see <see
	///  cref="SampleMultiPointList.XData" /> and <see cref="SampleMultiPointList.YData" />.
	/// </summary>
	...
	/// <summary>
	///  Sample data structure containing a variety of data values, in this case the values are
	///  related in that they correspond to the same time value.
	/// </summary>
	...
	/// <summary>
	///  A sample class that holds an internal collection, and implements the <see cref="IPointList"
	///  /> interface so that it can be used by ZedGraph as curve data.
	/// </summary>
	/// <remarks>
	///  This particular class efficiently implements the data storage so that the class can be
	///  cloned without duplicating the data points. For example, you can create a <see
	///  cref="SampleMultiPointList" />, populate it with values, and set <see cref="XData" /> =
	///  <see cref="PerfDataType.Time" /> and <see cref="YData" /> = <see
	///  cref="PerfDataType.Distance" />. You can then clone this <see cref="SampleMultiPointList"
	///  /> to a new one, and set <see cref="YData" /> = <see cref="PerfDataType.Velocity" />. Each
	///  of these <see cref="SampleMultiPointList" />'s can then be used as an <see
	///  cref="ZedGraph.GraphPane.AddCurve(string,IPointList,Color)" /> argument, thereby plotting a
	///  distance vs time curve and a velocity vs time curve. There will still be only one copy of
	///  the data in memory.
	/// </remarks>
	[Serializable]
	public class SampleMultiPointList : IPointList
	{
		...
		/// <summary>
		///  The Copy Constructor. This method does NOT duplicate the data, it merely makes another
		///  "Window" into the same collection. You can make multiple copies and set the <see
		///  cref="XData" /> and/or <see cref="YData" /> properties to different values to plot
		///  different data, while maintaining only one copy of the original values.
		/// </summary>
		...
		/// <summary>
		///  Indexer to access the data. This gets the appropriate data and converts to the <see
		///  cref="PointPair" /> struct that is compatible with ZedGraph. The actual data returned
		///  depends on the values of <see cref="XData" /> and <see cref="YData" />.
		/// </summary>
		...
		public PointPair this[int index]
		{
			get
			{
				...
				if (index >= 0 && index < this.Count)
				{
					...
					// extract the values from the struct according to the user-set enum values of
					// XData and YData
					xVal = perfData[XData];
					yVal = perfData[YData];
				}
		...
		/// <summary>
		///  Insert the specified <see cref="PerformanceData" /> struct into the list at the
		...
		/// <summary>
		///  Remove the <see cref="PerformanceData" /> struct from the list at the specified ordinal
		///  location.
		/// </summary>
		/// <param name="index">
		///  The ordinal location of the <see cref="PerformanceData" /> struct to be removed
		/// </param>
