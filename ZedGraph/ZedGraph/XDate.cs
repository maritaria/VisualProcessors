...
{
	/// <summary>
	///  This struct encapsulates a date and time value, and handles associated calculations and
	///  conversions between various formats.
	/// </summary>
	/// <remarks>
	///  This format stored as a double value representing days since a reference date (XL date 0.0
	///  is December 30, 1899 at 00:00 hrs). Negative values are permissible, and the range of valid
	///  dates is from noon on January 1st, 4713 B.C. forward. Internally, the date calculations are
	...
	public struct XDate : IComparable
	{
		...
		/// <summary> The default format string to be used in <see cref="ToString()"/> when no
		/// format is provided </summary> public const string DefaultFormatStr = "&d-&mmm-&yy
		/// &hh:&nn";
		...
		/// <summary>
		///  The actual date value in MS Excel format. This is the only data field in the <see
		///  cref="XDate" /> struct.
		/// </summary>
		...
		/// </summary>
		/// <param name="xlDate">An XL Date value in floating point double format</param>
		...
		/// <summary>
		///  Construct a date class from a calendar date (year, month, day). Assumes the time of day
		///  is 00:00 hrs
		/// </summary>
		/// <param name="year"> An integer value for the year, e.g., 1995.</param>
		/// <param name="day">  
		///  An integer value for the day of the month, e.g., 23. It is permissible to have day
		///  numbers outside of the 1-31 range, which will rollover to the previous or next month
		///  and year.
		/// </param>
		/// <param name="month">
		///  An integer value for the month of the year, e.g., 8 for August. It is permissible to
		///  have months outside of the 1-12 range, which will rollover to the previous or next
		///  year.
		/// </param>
		...
		/// </param>
		/// <param name="hour">  
		///  An integer value for the hour of the day, e.g. 15. It is permissible to have hour
		///  values outside the 0-23 range, which will rollover to the previous or next day.
		/// </param>
		/// <param name="minute">
		///  An integer value for the minute, e.g. 45. It is permissible to have hour values outside
		///  the 0-59 range, which will rollover to the previous or next hour.
		/// </param>
		/// <param name="second">
		///  An integer value for the second, e.g. 35. It is permissible to have second values
		///  outside the 0-59 range, which will rollover to the previous or next minute.
		/// </param>
		...
		/// </param>
		/// <param name="second">
		///  A double value for the second, e.g. 35.75. It is permissible to have second values
		///  outside the 0-59 range, which will rollover to the previous or next minute.
		/// </param>
		...
		///  An integer value for the millisecond, e.g. 632. It is permissible to have millisecond
		///  values outside the 0-999 range, which will rollover to the previous or next second.
		/// </param>
		...
		/*
				...
				/// <summary>
				///  Implement the <see cref="ICloneable" /> interface in a typesafe manner by
				///  justcalling the typed version of <see cref="Clone" />
				/// </summary>
		...
		*/
		...
		///  Astronomical Julian Day number, so a value of 0.0 corresponds to noon GMT on January
		///  1st, -4712. Thus, Julian Day number 2,400,000.0 corresponds to noon GMT on November 16,
		///  1858.
		/// </summary>
		...
		/// <returns>
		///  An XLDate value that lies between the minimum and maximum valid date ranges (see <see
		///  cref="XLDayMin" /> and <see cref="XLDayMax" />)
		/// </returns>
		...
		/// <summary>
		///  Get the calendar date (year, month, day, hour, minute, second) corresponding to this
		///  instance.
		/// </summary>
		/// <param name="year">  An integer value for the year, e.g., 1995.</param>
		/// <param name="day">   An integer value for the day of the month, e.g., 23.</param>
		...
		/// <summary>
		///  Get the day of year value (241.345 means the 241st day of the year) corresponding to
		///  this instance.
		/// </summary>
		...
		/// <summary>
		///  Calculate an Astronomical Julian Day number from the specified Calendar date (year,
		///  month, day, hour, minute, second), first normalizing all input data values
		/// </summary>
		...
		/// </param>
		/// <param name="hour">  The integer hour value (e.g., 14 for 2:00 pm).</param>
		/// <param name="minute">
		...
		/// </param>
		/// <returns>
		///  The corresponding Astronomical Julian Day number, expressed in double floating point
		///  format
		/// </returns>
		public static double CalendarDateToJulianDay(int year, int month, int day,
			int hour, int minute, int second)
		{
			// Normalize the data to allow for negative and out of range values In this way, setting
			// month to zero would be December of the previous year, setting hour to 24 would be the
			// first hour of the next day, etc.
			double ms = 0;
		...
		///  calendar is really only valid from October 15, 1582 forward. The countries that adopted
		///  the Gregorian calendar first did so on October 4, 1582, so that the next day was
		///  October 15, 1582. Prior to that time the Julian Calendar was used. However, Prior to
		///  March 1, 4 AD the treatment of leap years was inconsistent, and prior to 45 BC the
		///  Julian Calendar did not exist. The <see cref="XDate" /> struct projects only Gregorian
		///  dates backwards and does not deal with Julian calendar dates at all. The <see
		///  cref="ToString(double,string)" /> method will just append a "(BC)" notation to the end
		///  of any dates prior to 1 AD, since the <see cref="DateTime" /> struct throws an
		///  exception when formatting earlier dates.
		/// </remarks>
		/// <param name="year">       The integer year value (e.g., 1994).</param>
		/// <param name="month">      The integer month value (e.g., 7 for July).</param>
		/// <param name="day">        
		...
		/// </summary>
		/// <param name="dt">The date value in the form of a .Net DateTime struct</param>
		/// <returns>The corresponding XL Date, expressed in double floating point format</returns>
		...
		/// </summary>
		/// <param name="yearDec">The decimal year value in floating point double format.</param>
		/// <returns>The corresponding XL Date, expressed in double floating point format</returns>
		...
		/// </summary>
		/// <param name="jDay">  The Astronomical Julian Day number to be converted</param>
		/// <param name="year">  The integer year value (e.g., 1994).</param>
		/// <param name="month"> The integer month value (e.g., 7 for July).</param>
		/// <param name="day">   
		///  The integer day value (e.g., 19 for the 19th day of the month).
		...
		/// </param>
		/// <param name="millisecond">
		///  The <see cref="System.Double" /> millisecond value (e.g., 342.5 for 342.5 milliseconds
		///  past the second).
		/// </param>
		public static void JulianDayToCalendarDate(double jDay, out int year, out int month,
		...
		///  specified XL date
		/// </summary>
		/// <param name="xlDate">The XL date value in floating point double format.</param>
		/// <param name="year">  The integer year value (e.g., 1994).</param>
		/// <param name="month"> The integer month value (e.g., 7 for July).</param>
		/// <param name="day">   
		///  The integer day value (e.g., 19 for the 19th day of the month).
		...
		/// <summary>
		///  Calculate a day-of-week value (e.g., Sun=0, Mon=1, Tue=2, etc.) corresponding to the
		///  specified XL date
		/// </summary>
		/// <param name="xlDate">The XL date value in floating point double format.</param>
		...
		/// <summary>
		///  Calculate an Astronomical Julian Day Number from the specified Calendar date (year,
		///  month, day, hour, minute, second). This is the internal trusted version, where all
		///  values are assumed to be legitimate ( month is between 1 and 12, minute is between 0
		///  and 59, etc. )
		/// </summary>
		/// <param name="year">       The integer year value (e.g., 1994).</param>
		/// <param name="month">      The integer month value (e.g., 7 for July).</param>
		...
		{
			// Taken from http://www.srrb.noaa.gov/highlights/sunrise/program.txt routine calcJD()
		...
		/// <summary>
		///  Normalize a set of Calendar date values (year, month, day, hour, minute, second) to
		///  make sure that month is between 1 and 12, hour is between 0 and 23, etc.
		/// </summary>
		/// <param name="year">       The integer year value (e.g., 1994).</param>
		/// <param name="month">      The integer month value (e.g., 7 for July).</param>
		/// <param name="day">        
		...
		/// <summary>
		///  Add the specified number of milliseconds (can be fractional) to the current XDate
		///  instance.
		/// </summary>
		/// <param name="dMilliseconds">
		///  The incremental number of milliseconds (negative or positive) in floating point double
		///  format.
		/// </param>
		...
		/// </summary>
		/// <param name="dMinutes">
		///  The incremental number of minutes (negative or positive) in floating point double
		///  format.
		/// </param>
		...
		/// </summary>
		/// <param name="dMonths">
		///  The incremental number of months (negative or positive) in floating point double
		///  format.
		/// </param>
		...
		/// </summary>
		/// <param name="dSeconds">
		///  The incremental number of seconds (negative or positive) in floating point double
		///  format.
		/// </param>
		...
		/// <returns>
		///  A <see cref="DateTime" /> struct representing the specified xDate value.
		/// </returns>
		...
		/// <returns>
		///  An <see cref="XDate" /> struct representing the specified DateTime value.
		/// </returns>
		...
		/// <summary>
		///  '+' operator overload. When a double value is added to an XDate, the result is a new
		///  XDate with the number of days added.
		/// </summary>
		/// <param name="lhs">The left-hand-side of the '-' operator (an XDate class)</param>
		/// <param name="rhs">The right-hand-side of the '+' operator (a double value)</param>
		...
		/// </summary>
		/// <remarks>
		///  This method will throw an exception if <paramref name="target" /> is not an <see
		///  cref="XDate" /> object.
		/// </remarks>
		...
		/// <summary>
		///  Tests whether <param>obj</param> is either an <see cref="XDate" /> structure or a
		///  double floating point value that is equal to the same date as this <c>XDate</c> struct
		///  instance.
		/// </summary>
		/// <param name="obj">
		///  The object to compare for equality with this XDate instance. This object should be
		///  either a type XDate or type double.
		/// </param>
		/// <returns>
		///  Returns <c>true</c> if <param>obj</param> is the same date as this instance; otherwise,
		///  <c>false</c>
		/// </returns>
		...
		/// <summary>
		///  Returns the hash code for this <see cref="XDate" /> structure. In this case, the hash
		///  code is simply the equivalent hash code for the floating point double date value.
		/// </summary>
		...
		/// <summary>
		///  Format the specified XL Date value using the specified format string. The format string
		///  is specified according to the <see cref="DateTime" /> class.
		/// </summary>
		/// <remarks>
		///  The formatting is done using the <see cref="DateTime" /><see
		///  cref="System.DateTime.ToString(String)" /> method in order to provide full localization
		///  capability. The DateTime struct is limited to dates from 1 AD onward. However, all
		///  calendar dates in <see cref="XDate" /> and <see cref="DateTime" /> are projected
		///  Gregorian calendar dates. Since the Gregorian calendar was not implemented until
		///  October 4, 1582 (or later in some countries), Gregorian dates prior to that time are
		///  really dates that would have been, had the Gregorian calendar existed. In order to
		///  avoid throwing an exception, for dates prior to 1 AD, the year will be converted to a
		///  positive year and the text "(BC)" is appended to the end of the formatted string. Under
		///  this mode, the year sequence is 2BC, 1BC, 1AD, 2AD, etc. There is no year zero.
		/// </remarks>
		/// <param name="xlDate">
		///  The XL date value to be formatted in floating point double format.
		/// </param>
		/// <param name="fmtStr">
		///  The formatting string to be used for the date. See <see
		///  cref="System.Globalization.DateTimeFormatInfo" /> for a list of the format types
		///  available.
		/// </param>
		...
		/// <summary>
		///  Format this XDate value using the default format string (see
		///  cref="DefaultFormatStr"/&gt;).
		/// </summary>
		/// <remarks>
		///  The formatting is done using the <see cref="DateTime" /><see
		///  cref="System.DateTime.ToString(String)" /> method in order to provide full localization
		...
		/// <summary>
		///  Format this XL Date value using the specified format string. The format string is
		///  specified according to the <see cref="DateTime" /> class.
		/// </summary>
		/// <remarks>
		///  The formatting is done using the <see cref="DateTime" /><see
		///  cref="System.DateTime.ToString(String)" /> method in order to provide full localization
		...
		/// <param name="fmtStr">
		///  The formatting string to be used for the date. See <see
		///  cref="System.Globalization.DateTimeFormatInfo" /> class for a list of the format types
		///  available.
		/// </param>
		/// <returns>A string representation of the date</returns>
		...
		/*
				...
				/// </summary>
				/// <param name="fmtStr">
				///  The formatting string to be used for the date. The following formatting
