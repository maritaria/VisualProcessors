...
namespace ZedGraph
{
	...
	public class ValueHandler
	{
		...
		/// <summary>
		///  Basic constructor that saves a reference to the parent <see cref="GraphPane" /> object.
		/// </summary>
		/// <param name="pane">      The parent <see cref="GraphPane" /> object.</param>
		/// <param name="initialize">
		///  A <see cref="bool" /> flag to indicate whether or not the drawing variables should be
		///  initialized. Initialization is not required if this is part of a ZedGraph internal draw
		///  operation (i.e., its in the middle of a call to <see cref="GraphPane.Draw" />).
		///  Otherwise, you should initialize to make sure the drawing variables are configured.
		///  true to do an initialization, false otherwise.
		/// </param>
		...
		/// </summary>
		/// <remarks>
		///  The main purpose of this method is to handle stacked bars and lines, in which case the
		///  stacked values are returned rather than the individual data values. However, this
		///  method works generically for any curve type.
		/// </remarks>
		/// <param name="pane">   The parent <see cref="GraphPane" /> object.</param>
		...
		/// </param>
		/// <returns>
		///  true if the data point is value, false for <see cref="PointPairBase.Missing" />,
		///  invalid, etc. data.
		/// </returns>
							...
							out double baseVal, out double lowVal, out double hiVal)
		{
						...
						pane._barSettings.Type == BarType.PercentStack))
			{
				...
				// loop through all the curves, summing up the values to get a total (only for the
				...
				foreach (CurveItem tmpCurve in pane.CurveList)
				{
					...
					if (tmpCurve.IsBar && tmpCurve.IsVisible)
					{
						...
						// otherwise, it's an ordinal type so use the value at the same ordinal
						// position
			...
			// If the curve is a stacked line type, then sum up the values similar to the stacked
			// bar type
				...
				foreach (CurveItem tmpCurve in pane.CurveList)
				{
					...
					if (tmpCurve is LineItem && tmpCurve.IsVisible)
					{
						...
						if (tmpCurve == curve)
						{
							...
							// if ( curVal < 0 && stack == 0 ) { stack = curVal; lowVal = curVal;
							// hiVal = curVal; } else
		...
		/// <summary>
		///  Calculate the user scale position of the center of the specified bar, using the <see
		///  cref="Axis" /> as specified by <see cref="BarSettings.Base" />. This method is used
		///  primarily by the <see cref="GraphPane.FindNearestPoint(PointF,out CurveItem,out int)"
		///  /> method in order to determine the bar "location," which is defined as the center of
		///  the top of the individual bar.
		/// </summary>
		/// <param name="curve">   
		///  The <see cref="CurveItem" /> representing the bar of interest.
		/// </param>
		/// <param name="barWidth">
		///  The width of each individual bar. This can be calculated using the <see
		///  cref="CurveItem.GetBarWidth" /> method.
		/// </param>
		/// <param name="iCluster">
		///  The cluster number for the bar of interest. This is the ordinal position of the current
		///  point. That is, if a particular <see cref="CurveItem" /> has 10 points, then a value of
		///  3 would indicate the 4th point in the data array.
		/// </param>
		/// <param name="val">     The actual independent axis value for the bar of interest.</param>
		/// <param name="iOrdinal">
		///  The ordinal position of the <see cref="CurveItem" /> of interest. That is, the first
		///  bar series is 0, the second is 1, etc. Note that this applies only to the bars. If a
		///  graph includes both bars and lines, then count only the bars.
		/// </param>
		...
		/// </summary>
		/// <remarks>
		///  The main purpose of this method is to handle stacked bars, in which case the stacked
		///  values are returned rather than the individual data values.
		/// </remarks>
		/// <param name="curve">  A <see cref="CurveItem" /> object of interest.</param>
		/// <param name="iPt">    The zero-based point index for the point of interest.</param>
		/// <param name="baseVal">
