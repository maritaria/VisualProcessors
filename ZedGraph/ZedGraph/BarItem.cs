...
namespace ZedGraph
{
	...
	/// </summary>
	/// <remarks>
	///  The orientation of the bars depends on the state of <see cref="BarSettings.Base" />, and
	///  the bars can be stacked or clustered, depending on the state of <see
	///  cref="BarSettings.Type" />
	/// </remarks>
	...
	public class BarItem : CurveItem, ICloneable, ISerializable
	{
		...
		/// <summary>
		///  Private field that stores a reference to the <see cref="ZedGraph.Bar" /> class defined
		///  for this <see cref="BarItem" />. Use the public property <see cref="Bar" /> to access
		///  this value.
		/// </summary>
		protected Bar _bar;
		...
		/// <summary>
		///  Gets a reference to the <see cref="ZedGraph.Bar" /> class defined for this <see
		///  cref="BarItem" />.
		/// </summary>
		public Bar Bar
		{
		...
		/// <param name="label">The label that will appear in the legend.</param>
		/// <param name="x">    
		///  An array of double precision values that define the independent (X axis) values for
		///  this curve
		/// </param>
		/// <param name="y">    
		///  An array of double precision values that define the dependent (Y axis) values for this
		///  curve
		/// </param>
		/// <param name="color">
		///  A <see cref="Color" /> value that will be applied to the <see cref="ZedGraph.Bar.Fill"
		///  /> and <see cref="ZedGraph.Bar.Border" /> properties.
		/// </param>
		...
		/// <param name="label"> The label that will appear in the legend.</param>
		/// <param name="points">
		///  A <see cref="IPointList" /> of double precision value pairs that define the X and Y
		///  values for this curve
		/// </param>
		/// <param name="color"> 
		///  A <see cref="Color" /> value that will be applied to the <see cref="ZedGraph.Bar.Fill"
		///  /> and <see cref="ZedGraph.Bar.Border" /> properties.
		...
		///  vertical bars in clusters or stacks, but only for <see cref="BarItem" /> types. This
		///  method does not apply to <see cref="ErrorBarItem" /> or <see cref="HiLowBarItem" />
		///  objects. Call this method only after calling <see cref="GraphPane.AxisChange()" />.
		/// </remarks>
		/// <param name="pane">       The GraphPane in which to place the text labels.</param>
		/// <param name="isBarCenter">
		///  true to center the labels inside the bars, false to place the labels just above the top
		///  of the bar.
		/// </param>
			...
			bool isUnderline)
		{
			...
			foreach (CurveItem curve in pane.CurveList)
			{
				...
				if (bar != null)
				{
					...
					// ADD JKB 9/21/07 The labelOffset should depend on whether the curve is YAxis
					// or Y2Axis. JHC - Generalize to any value axis Make the gap between the bars
					// and the labels = 1.5% of the axis range
					float labelOffset;
					...
					{
						// Get the high, low and base values for the current bar note that this
						// method will automatically calculate the "effective" values if the bar is
						// stacked
						double baseVal, lowVal, hiVal;
						valueHandler.GetValues(curve, i, out baseVal, out lowVal, out hiVal);

						// Get the value that corresponds to the center of the bar base This method
						// figures out how the bars are positioned within a cluster
						...
						// data for this, since hiVal and lowVal could be "effective" values from a
						// bar stack
						string barLabelText = (isVertical ? points[i].Y : points[i].X).ToString(valueFormat);

						// Calculate the position of the label -- this is either the X or the Y
						// coordinate depending on whether they are horizontal or vertical bars,
						// respectively
						float position;
						if (isBarCenter)
						...
						// CHANGE JKB 9/21/07 CoordinateFrame should depend on whether curve is
						// YAxis or Y2Axis.
		...
		/// <summary>
		///  Do all rendering associated with this <see cref="BarItem" /> to the specified <see
		...
		/// </param>
		/// <param name="pos">        
		///  The ordinal position of the current <see cref="Bar" /> curve.
		/// </param>
		...
		override public bool GetCoords(GraphPane pane, int i, out string coords)
		{
			...
			// pixBase = pixel value for the bar center on the base axis pixHiVal = pixel value for
			// the bar top on the value axis pixLowVal = pixel value for the bar bottom on the value
			// axis
			...
			// curBase = the scale value on the base axis of the current bar curHiVal = the scale
			// value on the value axis of the current bar curLowVal = the scale value of the bottom
			// of the bar
			double curBase, curLowVal, curHiVal;
			ValueHandler valueHandler = new ValueHandler(pane, false);
			valueHandler.GetValues(this, i, out curBase, out curLowVal, out curHiVal);

			// Any value set to double max is invalid and should be skipped This is used for
			// calculated values that are out of range, divide by zero, etc. Also, any value <= zero
			// on a log scale is invalid
